            ...
            <div class="msg-container" v-for="msg in allMessages" :key="msg.text">
                <div class="single-msg d-flex flex-column">
                    <div class="msg-userinfo d-inline-flex">
                        <img :src="`http://${HOST}:3001/user/get_avatar?avatar=` + msg.avatar" class="avatar-msg"
                            alt="avatar">
...
async function getDmInfo(): Promise<void> {
    await axios
        ...
        .then(async (response) => {
            ...
            dmAvatar.value = `http://${HOST}:3001/user/get_avatar?avatar=` + thisDm[0].otherUser.avatar
        })
        .catch((error: any) => {
